...
Client.prototype.__setup_master = function (cb) {
  ...
  socket.on('connect', function () {
    ...
    socket.once('data', function (data) {
      ...
      socket.on('data', self.__pre_request.bind(self));
...
Client.prototype.__pre_request = function (data) {
  if (data.toString() === 'pong') {
    return;
  }

  var arr_data = '[' + data.toString().replace(/}(\s+)?{/g, '}, {') + ']';

  var requests;
  try {
    requests = JSON.parse(arr_data.toString());
  } catch (e) {
    console.log(e);
  ...
  requests.forEach(this.__create_pool_connection.bind(this));
};

Client.prototype.__create_pool_connection = function (request) {
  var self = this;
